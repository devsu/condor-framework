...
module.exports = class {
  constructor(builder, options) {
    ...
    if (this._areSslOptions(options)) {
      this._validateSslOptions(options);
      options.creds = this._getOptionsWithSslCredentials(options);
    }
  ...
  _areSslOptions(options) {
    return options && (options.rootCert || options.certChain || options.privateKey);
  }

  _validateSslOptions(options) {
    if ((options.rootCert || options.certChain || options.privateKey) &&
       !(options.rootCert && options.certChain && options.privateKey)) {
      throw new Error('Cannot perform operation: rootCert, privateKey and certChain' +
        ' are required when using ssl');
    }
  }

  _getOptionsWithSslCredentials(options) {
    return grpc.ServerCredentials.createSsl(this._getFileBuffer(options.rootCert), [
      {
  ...
  _getFileBuffer(path) {
    if (!fs.existsSync(path)) {
      throw new Error('Cannot perform operation: Ssl certificate files not found');
    }
