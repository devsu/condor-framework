...
module.exports = class {
  ...
  _buildProxyMethod(propertyName, middleware, errorHandlers) {
    ...
    return (call, callback) => {
      return this._executeMiddleware(middleware, call).then((result) => {
        ...
        let error = e;
        return this._executeErrorHandlers(errorHandlers, error, call).then((result) => {
          if (result && callback) {
            return callback(null, result);
          }
          if (!error) {
  ...
  _executeMiddleware(middleware, call) {
  ...
  _executeErrorHandlers(errorHandlers, error, call) {
    let promise = Promise.resolve();
    ...
    for (let i = 0; i < errorHandlers.length; i++) {
      promise = promise.then((result) => {
        ...
        return errorHandlers[i].method(error, call);
      });
    }
    return promise;
  }
