...
module.exports = class {
  constructor(serviceDefinition, middleware, errorHandlers) {
    ...
    const propertiesNames = this._getAllPropertiesNames(this._serviceDefinition.implementation);
  ...
  _getAllPropertiesNames(implementation) {
    const propertyNames = [];
    let nextClass = implementation;
    while (Object.getPrototypeOf(nextClass) instanceof Object) {
      const nextPrototype = Object.getPrototypeOf(nextClass);
      let prototypePropertyNames = Object.getOwnPropertyNames(nextPrototype);

      prototypePropertyNames = prototypePropertyNames.filter((propertyName) => {
        return propertyName !== 'constructor';
      });

      propertyNames.push(...prototypePropertyNames);
      nextClass = nextPrototype;
    }
