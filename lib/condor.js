...
module.exports = class {
  ...
  addService() {
    this._validateServerHasNotStarted('service');
    this._builder.addService(...arguments);
    return this;
  }

  addMiddleware() {
    this._validateServerHasNotStarted('middleware');
    this._builder.addMiddleware(...arguments);
    return this;
  }

  addErrorHandler() {
    this._validateServerHasNotStarted('error handler');
  ...
  _validateServerHasNotStarted(context) {
    if (this._hasStarted()) {
      throw new Error(`Cannot add ${context}: Server has already started`);
    }
  }
