...
describe('Server:', () => {
  ...
  describe('start()', () => {
    ...
    it('should pass the ssl credentials to grpc server', () => {
      ...
      server = new Server(builder, {creds});
      server.start();
      expect(serverMock.bind).toHaveBeenCalledTimes(1);
      expect(serverMock.bind).toHaveBeenCalledWith(jasmine.any(String), creds);
    });

    it('should pass the call credentials to grpc server', () => {
      const callCredentials = grpc.credentials.createFromMetadataGenerator();
      const sslCreds = grpc.credentials.createSsl();
      const combineCreds = grpc.credentials.combineChannelCredentials(sslCreds, callCredentials);
      server = new Server(builder, {'creds': combineCreds});
      server.start();
      expect(serverMock.bind).toHaveBeenCalledTimes(1);
      expect(serverMock.bind).toHaveBeenCalledWith(jasmine.any(String), combineCreds);
    });
  });
