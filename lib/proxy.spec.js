  ...
  describe('each middleware', () => {
    ...
    describe('when next() is called', () => {
      it('should call the implementation', (done) => {
        middlewareMock.methodMiddleware.method.and.callFake((context, next) => {
          expect(personServiceMock.list).not.toHaveBeenCalled();
          next();
        });
        proxy.list(call, () => {
          expect(middlewareMock.methodMiddleware.method).toHaveBeenCalled();
          done();
        });
      });
      ...
      it('should call the next middleware', (done) => {
        ...
        middlewareMock.packageMiddleware.method.and.callFake((context, next) => {
          expect(middlewareMock.globalMiddleware.method).toHaveBeenCalled();
          expect(personServiceMock.list).not.toHaveBeenCalled();
          next();
        });
        proxy.list(call, () => {
      ...
      xit('should return the value on the next', (done) => {
        const expectedResponse = new Response({'message': 'message'});
